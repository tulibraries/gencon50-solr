...
RESP=$(curl -s -o /dev/null -w "%{http_code}" -X POST -H "Authorization: token $GITHUB_TOKEN" --data-binary @$HOME/solrconfig.zip -H "Content-Type: application/octet-stream" "https://uploads.github.com/repos/tulibraries/gencon50-solr/releases/$RELEASE_ID/assets?name=gencon50-$RELEASE_TAG.zip")
validate_create
